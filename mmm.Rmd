...
summary(model)
# ggeffects::ggpredict(model) %>% plot() # plot expected counts
```
...
```{r eval=FALSE}
heterovar_res = glmmTMB(Reaction ~ Days + (1|Subject) + diag(0 + factor(Days) |Subject), 
                         data = lme4::sleepstudy %>% filter(Days %in% c(0,4,9)))
VarCorr(heterovar_res)
```
...
model = stan_lmer(giniPercap ~ year + (1 + year|country), 
                  recent_europe, 
                  cores=4)
print(model, digits = 3)
qplot(data=ranef(model)$country, x=`(Intercept)`, y = year, geom='smooth')
ggeffects::ggpredict(model) %>% plot()
```
