...
The following shows the basic mgcv approach to a GAM.  Here we add a special approach to modeling the time trend, allowing it to be more flexible than a standard linear effect.  Behind the scenes the estimation approach strives to not be too 'wiggly', but does allow the data to better speak for itself.
...
In the case of mixed models, we don't necessarily have to explore such effects, but now we have a tool that would allow us to do so while adding random effects. Lets do a random intercept model with a country random effect.
...
Now, let's add a random coefficient for the year effect, and compare it to the corresponding `lme4` model.
...
The `lme4` package is a fantastic tool for mixed models, but it does have limitations that will likely not make it the best tool for some not uncommon data situations.  The packages demoed here provide enough options that you will be able to run just about any model you like when the time comes.